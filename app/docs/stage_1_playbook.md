...
Для реализации базовой аутентификации используются модели, определенные в `src/lib/database/db.contracts.ts`:
...
- **ADR:** Создан документ, объясняющий решение использовать аутентификацию по телефону без пароля на первом этапе и описывающий план перехода к полноценному RBAC/2FA. (См. [ADR-005](./01_architecture/ADR-005-passwordless-auth.md))
...
- [x] Весь серверный код находится в `*.server.ts`.
- [x] Ключевые события логируются; сообщения об ошибках локализованы (RU/EN).
- [x] Unit и интеграционные тесты покрывают основную логику и проходят в CI.
- [x] Созданы ADR и секвенс-диаграмма.
